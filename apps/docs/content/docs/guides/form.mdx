...
We'll now create the form. The form uses the [Upload Dropzone](/components/upload-dropzone) component to allow users to upload files. The form also has an input field for arbitrary text.
...
import { UploadedFile } from 'better-upload/client'; // [!code highlight]

export function MyForm() {
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>([]); // [!code highlight]
              ...
              <FormItem>
                ...
                <FormControl>
                  <UploadDropzone
                    ...
                    onUploadComplete={({ files }) => {
                      field.onChange(files.map((file) => file.objectKey));
                      setUploadedFiles(files); // [!code highlight]
                    }}
                    onUploadError={(error) => {
                      form.setError('objectKeys', {
                        message: error.message || 'An error occurred',

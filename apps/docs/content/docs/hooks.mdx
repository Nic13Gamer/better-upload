...
- `onSuccess`: A callback that is called after the file has been uploaded. Has the file, as well as metadata sent from the server.
- `onError`: A callback that is called if the upload fails. Has error details.

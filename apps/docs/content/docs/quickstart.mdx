...
Install the `better-upload` package, aswell as the AWS S3 Client.
...
In the example above, we create the upload route `demo` for single images. You can create multiple routes for different purposes, like uploading more than one file. Learn more about routes [here](/router#upload-routes).

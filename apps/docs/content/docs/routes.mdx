...
If you want to upload files larger than 5GB, you need to use multipart uploads. To enable it, set `multipart` to `true`.

It works both for single and multiple files.
...
You can also modify the part size by setting `partSize` in bytes. The default is 50MB.

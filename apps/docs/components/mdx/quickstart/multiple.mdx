...
The available S3-compatible clients are: `aws()`, `cloudflare()`, `backblaze()`, `tigris()`, `digitalOcean()`, `wasabi()`, and `minio()`. Suggest a new client by [opening an issue](https://github.com/Nic13Gamer/better-upload/issues).

There is also `custom()`, which allows any S3-compatible service to be configured.
